...
import { useCreateOpposingParty } from "../hooks/useOpposingParties";
import { checkDuplicates } from "../api/opposing-party.api";
import { CreateOpposingPartyRequest, Duplicate } from "../types/opposing-party.types";
import { useCurrentUserPermissions } from "@/modules/roles/hooks/useCurrentUserPermissions";

export default function CreateOpposingPartyPage() {
  const navigate = useNavigate();
  const createMutation = useCreateOpposingParty();
  const { permissions, isLoading: isPermsLoading } = useCurrentUserPermissions();

  const [partyType, setPartyType] = useState<"INDIVIDUAL" | "ORGANIZATION">("INDIVIDUAL");
  const [formData, setFormData] = useState<CreateOpposingPartyRequest>({
  ...
  useEffect(() => {
    const nameInput = partyType === "INDIVIDUAL" ? `${formData.firstName} ${formData.lastName}`.trim() : formData.organizationName?.trim();
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full animate-fade-in">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-white">Add Opposing Party</h1>
        <p className="mt-2 text-sm text-surface-200/60">
          Create a new individual or organization opposing party record.
        </p>
      </div>
        ...
        <div className="lg:col-span-2 bg-surface-900/60 backdrop-blur-md border border-white/[0.06] rounded-2xl p-6 md:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2 border-t border-white/[0.04]">
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="e.g. lex@lexcorp.com"
                  className="w-full bg-surface-950 border border-white/[0.08] hover:border-white/[0.12] focus:border-brand-500/80 focus:ring-1 focus:ring-brand-500/80 rounded-xl px-4 py-2.5 text-sm text-white transition-all duration-200"
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="e.g. https://lexcorp.com"
                  className="w-full bg-surface-950 border border-white/[0.08] hover:border-white/[0.12] focus:border-brand-500/80 focus:ring-1 focus:ring-brand-500/80 rounded-xl px-4 py-2.5 text-sm text-white transition-all duration-200"
                />
              </div>
            </div>
            ...
            <div className="flex gap-4 justify-end pt-4 border-t border-white/[0.04]">
              ...
              >
                {createMutation.isPending ? "Creating..." : "Save Opposing Party"}
              </button>
            </div>
          </form>
        </div>
