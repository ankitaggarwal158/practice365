...
import { useOpposingParty, useUpdateOpposingParty } from "../hooks/useOpposingParties";
import { checkDuplicates } from "../api/opposing-party.api";
import { UpdateOpposingPartyRequest, Duplicate } from "../types/opposing-party.types";
import { useCurrentUserPermissions } from "@/modules/roles/hooks/useCurrentUserPermissions";

export default function EditOpposingPartyPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  
  const { data: opposingParty, isLoading: isPartyLoading } = useOpposingParty(id!);
  const updateMutation = useUpdateOpposingParty();
  const { permissions, isLoading: isPermsLoading } = useCurrentUserPermissions();

  const [formData, setFormData] = useState<UpdateOpposingPartyRequest>({
    firstName: "",
    lastName: "",
    organizationName: "",
    email: "",
  ...
  useEffect(() => {
    if (opposingParty) {
      setFormData({
        firstName: opposingParty.firstName || "",
        lastName: opposingParty.lastName || "",
        organizationName: opposingParty.organizationName || "",
        email: opposingParty.email || "",
        phone: opposingParty.phone || "",
        alternatePhone: opposingParty.alternatePhone || "",
        website: opposingParty.website || "",
        addressLine1: opposingParty.addressLine1 || "",
        addressLine2: opposingParty.addressLine2 || "",
        city: opposingParty.city || "",
        state: opposingParty.state || "",
        postalCode: opposingParty.postalCode || "",
        country: opposingParty.country || "",
        notes: opposingParty.notes || "",
      });
    }
  }, [opposingParty]);
  ...
  useEffect(() => {
    if (!opposingParty) return;
    const nameInput = opposingParty.partyType === "INDIVIDUAL" ? `${formData.firstName} ${formData.lastName}`.trim() : formData.organizationName?.trim();
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const result = await checkDuplicates({
          partyType: opposingParty.partyType,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateMutation.mutate(
      ...
      {
        onSuccess: () => {
          navigate(`/opposing-parties/${id}`);
        },
      }
    );
  };

  if (isPartyLoading || isPermsLoading) {
    return (
  ...
  if (!canManage) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 text-center text-danger font-semibold">
        Access Denied. You do not have permissions to manage opposing parties.
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full animate-fade-in">
      ...
      <div className="mb-8">
        <button
          onClick={() => navigate(`/opposing-parties/${id}`)}
          className="text-sm text-surface-200/50 hover:text-white transition-colors flex items-center gap-2 mb-4"
        >
          &larr; Back to Opposing Party Details
        </button>
        <h1 className="text-3xl font-bold tracking-tight text-white">Edit Opposing Party</h1>
        <p className="mt-2 text-sm text-surface-200/60">
          Modify records and attributes for this opposing party representation.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Duplicate Checking Warning Panel */}
        <div className="space-y-6">
          <div className="bg-surface-900/60 backdrop-blur-md border border-white/[0.06] rounded-2xl p-5 shadow-xl">
            <h3 className="text-sm font-bold text-white mb-3 uppercase tracking-wide flex justify-between items-center">
              <span>Conflict Checks Warnings</span>
