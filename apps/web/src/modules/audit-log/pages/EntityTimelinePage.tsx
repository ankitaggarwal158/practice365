...
import { useEntityTimeline } from "../hooks/useAuditLogs";
...
export default function EntityTimelinePage() {
  ...
  const { data: result, isLoading, error } = useEntityTimeline(id || "");
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 text-center text-danger font-semibold">
        {(error as any)?.response?.data?.message || "Failed to load entity timeline."}
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full animate-fade-in">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-white">Entity Activity Timeline</h1>
